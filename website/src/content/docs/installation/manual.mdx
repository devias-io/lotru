...
<Step>Create a `theme.ts` file</Step>

Create a `theme.ts` file in your project and add the following code:
...
<Step>Create a `pigment.config.ts` file</Step>
...
<Step>Import Pigment CSS configuration</Step>
...
<Step>Create a `lotru.config.json` file</Step>
